...
function showResults() {
  ...
  artists.forEach((artist, index) => {
    ...
    if (locationsFilterValue && locationsFilterValue !== "All") {
      const hasLocation = artist.Locations.some((location) =>
        location.toLowerCase().includes(locationsFilterValue)
      );
      if (!hasLocation) {
        cards[index].style.display = "none";
      }
    ...
    if (
      !(
        artist.creationDate >= creationDateRangeValue.min &&
        artist.creationDate <= creationDateRangeValue.max
      )
    ) {
      cards[index].style.display = "none";
    }
    ...
    if (
      !(
        firstAlbumYear >= firstAlbumRangeValue.min &&
        firstAlbumYear <= firstAlbumRangeValue.max
      )
    ) {
      cards[index].style.display = "none";
    }
...
document
  ...
  .addEventListener("input", searchChangeHandler); // Event listener to search input
...
}); // Delete artists relations since it's not wanted in suggestions
...
var clickedFilterButton = false;

if (toggleButton && filtersSlider && containerSection) {
  toggleButton.addEventListener("click", () => {
    const isActive = filtersSlider.classList.toggle("active");
    containerSection.classList.toggle("active", isActive);
  });
  ...
  document.addEventListener("keydown", (e) => {
    ...
    if (e.key === "/" && !e.ctrlKey && !e.metaKey) {
      ...
      var isActive = filtersSlider.classList.toggle("active");
      containerSection.classList.toggle("active", isActive);
      const filterDiv = document.getElementById("filterDiv");
      isActive = filterDiv.classList.toggle("active");
      filterDiv.classList.toggle("active", isActive);
    }
  });
} else {
  console.error("[containerSection] Makayench !!");
}

// ------------ Set innerHtml Filters ------------
document.getElementById("toggleFilterButton").addEventListener("click", () => {
  const filterDiv = document.getElementById("filterDiv");
  const isActive = filterDiv.classList.toggle("active");
...
locations.forEach(
  (item) =>
    (locationsFilter.innerHTML +=
      "<option class='locationsOption'>" + item.split(" - ")[0] + "</option>")
);
...
i = 1;
while (i <= maxMembers) {
  membersCountFilter.innerHTML +=
    "<input type='checkbox' id='" + i + "' checked/>" + i;
